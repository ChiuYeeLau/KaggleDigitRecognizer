...
def conv_neural_network (params): 
	## params = (learning_rate, num_train_sample, num_conv_layer, num_feature_map, mini_batch_size, rand_seed, train_input, train_output, raw_train_output, test_input, num_test_sample, image_size, patch_size, pool_size, full_connected_hidden_units, output_type, max_iter)

	learning_rate, num_train_sample, num_conv_layer, num_feature_map, mini_batch_size, rand_seed, train_input, train_output, raw_train_output, test_input, num_test_sample, image_size, patch_size, pool_size, full_connected_hidden_units, output_type, max_iter = params
	...
	layer[0] = LeNetConvPoolLayer (rand_generator, input = layer_input, image_shape = (mini_batch_size, 1, image_size, image_size), filter_shape = (num_feature_map[0], 1, patch_size, patch_size), poolsize = (pool_size, pool_size))

	updated_image_size = (image_size - patch_size + 1)/pool_size

	for loop in range (1, num_conv_layer):
		layer[loop] = LeNetConvPoolLayer (rand_generator, input = layer[loop - 1].output, image_shape = (mini_batch_size, num_feature_map[loop - 1], updated_image_size, updated_image_size), filter_shape = (num_feature_map[loop], num_feature_map[loop - 1], patch_size, patch_size), poolsize = (pool_size, pool_size))
	...
	fully_connected = HiddenLayer(rand_generator, input = fully_connected_input, n_in = num_feature_map [num_conv_layer - 1] * updated_image_size * updated_image_size, n_out = full_connected_hidden_units, activation = T.tanh)
	fully_connected_output = LogisticRegression (input = fully_connected.output, n_in = full_connected_hidden_units, n_out = output_type)
...
learning_rate = 0.001
num_conv_layer = 2
num_feature_map = (50, 100)
mini_batch_size = 1
rand_seed = 86027768
image_size = 28
patch_size = 5
pool_size = 2
full_connected_hidden_units = 500
output_type = 10
max_iter = 300
